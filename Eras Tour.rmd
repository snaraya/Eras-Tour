...
```{r}
write_csv(Taylor2, "input/Taylor2.csv")
```
...
gg_opener <- df_opener |> 
  ...
  theme(
    ...
    plot.title = element_text(family = "Soulmate", hjust = 0.5, size = 12),
    plot.subtitle = element_text(hjust = 0.5, size = 10),
    legend.title = element_text(family = "Soulmate", size = 10),
...
Para guardar el gráfico, utilizamos `ggsave()`:

```{r}

ggsave("output/teloneros_principales.png", 
       gg_opener,
       width = 5, height = 6)
...
gg_todos_teloneros <- df_teloneros_total |>
...
```{r}
ggsave("output/todos_teloneros.png", 
       gg_todos_teloneros,
       width = 4, 
       height = 5,
       dpi = 200)  
...
```{r}
ggsave("output/show_totales.png", width = 9, height = 7)
```
